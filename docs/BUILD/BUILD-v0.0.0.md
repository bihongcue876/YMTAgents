...
## 31. rev29 轮次记录 — 显示体验修正（2026-09-19）

用户反馈：空状态两行短句被折成四行；右栏详情默认宽度偏窄、按钮文字被挤。

### 31.1 变更

| # | 问题 | 修复 | 落点 |
|---|---|---|---|
| 1 | 空状态「本地优先…」「模型已就绪…」各折两行（共四行） | 换行标签不设 `alignment`，占满布局宽度 | `gui/chat/empty_state.py` |
| 2 | 右栏详情默认 320 / 最小 280 偏窄 | 默认 380、钳制 340–560 | `gui/main_window.py` |

### 31.2 冒烟结果

| 项 | 结果 |
|---|---|
| 全量测试 | **212 passed, 2 skipped，退出码 0**（211+2 → 212+2，净增 1，只增不减） |
| 新增用例 | 空状态短句在宽布局下各占一行 |


## 32. rev30 轮次记录 — 切片 1：右栏问题列表（2026-09-19）

用户裁决（m0575/m0589/m0591）：问题列表全选四项 —— 轮次编号并高亮当前 / 每项右键操作 /
显示更完整内容 / 可折叠或可搜索。本轮先做**显示层**（无协议变更）；右键的
「退回到此前 / 从此处分支」（m0591 定名）需分支/revert 后端，与其一起在**切片 2** 落地，
避免上线不可用的菜单项。
...
| # | 变更 | 落点 |
|---|---|---|
| 1 | 「第 N 轮 + 完整提问」；不再 60 字截断（`_questions` 仅折换行） | `gui/main_window.py` |
| 2 | 末条 = 当前所在位置，加粗高亮 + tooltip 标注 | `gui/chat/session_panel.py` |
| 3 | `setWordWrap(True)`；长提问完整显示、tooltip = 全文 | `gui/chat/session_panel.py` |
| 4 | 搜索框子串过滤 + 折叠/展开切换 | `gui/chat/session_panel.py` |
| 5 | 列表项以 `Qt.UserRole` 存原始序号，过滤后跳转仍正确 | `gui/chat/session_panel.py` |

### 32.2 冒烟结果

| 项 | 结果 |
|---|---|
| 全量测试 | **213 passed, 2 skipped，退出码 0**（212+2 → 213+2，净增 1，只增不减） |
| 新增用例 | 轮次编号 / 当前高亮 / 完整内容不截断 / 搜索过滤后按原始序号跳转 / 折叠切换 |
| 门禁 | 依赖方向 / 契约 / 主题与字号纪律 全部通过 |

### 32.3 后续（切片 2）

分支树与 revert：`sessions/<id>/graph.json`（分支只存事件序号 + 光标，`events.jsonl` 不复制）、
`session.branch/revert/switch_branch` 契约、`SessionBranches` 事件、按分支的摘要、右栏「分支」段
